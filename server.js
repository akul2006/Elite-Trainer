...
app.post('/api/inquire', async (req, res) => {
  ...
  try {
    ...
    `;
    const values = [
      name, email, subject, message, companyName || null, 
      teamSize ? parseInt(teamSize, 10) : null, linkedinUrl || null, institutionName || null
    ];
    await pool.query(queryText, values);

  } catch (error) {
    console.error('Database transaction failed:', error);
    return res.status(500).json({ success: false, message: 'Failed to save inquiry to database.' });
  }
